...
func rotate_towards_target(rtarget, delta):
	#if rtarget == null:
		#return
	var target_vector = $Catapult.global_position.direction_to(Vector3(rtarget.global_position.x, global_position.y, rtarget.global_position.z))
	var target_basis:Basis = Basis.looking_at(target_vector)
	$Catapult.basis = $Catapult.basis.slerp(target_basis, acquire_slerp_progress)
...
func _on_attacking_state_state_physics_processing(_delta):
	...
	if current_enemy != null and enemies_in_range.has(current_enemy):
		...
		$Catapult.look_at(current_enemy.global_position)
		# If cooldown over
...
func fire():
	...
	projectile.starting_position = $Catapult/projectile_spawn.global_position
